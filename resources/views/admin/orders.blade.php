<?php
	use Carbon\Carbon;
	if(url()->current()==url('admin/orders/filter_date')){
		$start=to_j2($start);
		$end=to_j2($end);
	}else{
				...
				<div class="x_content">
					<div class="tile_count">
						<div class="col-md-4 col-sm-4 col-xs-12 tile_stats_count">
							<h4>تعداد</h4>
							<h5>{{number2farsi2($reports["orders"])}} سفارش</h5>
						</div>
						<div class="col-md-4 col-sm-4 col-xs-12 tile_stats_count">
						...
						<div class="col-md-4 col-sm-4 col-xs-12 tile_stats_count">
							<h4>سود (تحویل شده)</h4>
							<h5>{{number2farsi($reports["profit"])}}  ریال</h5>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="x_panel tile">
				...
				<div class="x_content">
					<div class="tile_count">
						...
						<div class="col-md-2 col-sm-2 col-xs-6 tile_stats_count">
							<h4>ثبت اولیه</h4>
							<h5>{{number2farsi2($reports["orders_init"])}} سفارش</h5>
						</div>
						<div class="col-md-2 col-sm-2 col-xs-6 tile_stats_count">
							<h4>آماده ارسال</h4>
							<h5>{{number2farsi2($reports["orders_ready"])}} سفارش</h5>
						</div>
						<div class="col-md-2 col-sm-2 col-xs-6 tile_stats_count blue">
							<h4>ارسال شده</h4>
							<h5>{{number2farsi2($reports["orders_sent"])}} سفارش</h5>
						</div>
						<div class="col-md-2 col-sm-2 col-xs-6 tile_stats_count green">
							<h4>تحویل شده</h4>
							<h5>{{number2farsi2($reports["orders_delivered"])}} سفارش</h5>
						</div>
						<div class="col-md-2 col-sm-2 col-xs-6 tile_stats_count red">
							<h4>منقضی شده</h4>
							<h5>{{number2farsi2($reports["orders_expired"])}} سفارش</h5>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div class="x_panel tile">
				...
				<div class="x_content">
					<form action="{{url('/admin/orders/filter_date')}}" method="post" class="form-inline">
						{{ csrf_field() }}
											...
											<div class="panel-heading" role="tab" id="heading{{$i}}">
												<div class="panel-title">
													<a class="collapsed" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse{{$i}}" aria-expanded="false" aria-controls="collapse{{$i}}">
														<div style="font-size:12px;"><i style="font-size:15px;" class="icon-chevron-down"></i> {{"   ".$order["customer_name"]." - ".to_j($order["order_date"])." - ".number2farsi2($order["order_time"])." - ".$order["vendor_name"]}}</div>
													</a>
												</div>
											</div>
													...
													<table class="table table-hover" id="cart-table">
														...
														<tbody>
															<tr>
																...
																<td>
																	@if($order["status"]==0)
																		<a href="{{url('admin/orders/expired/'.$order['basket_id'])}}"><button class="btn btn-md btn-danger my-update"><i class="icon-ok"> </i>لغو کن</button></a>
																	@endif
																</td>
															</tr>
														</tbody>
